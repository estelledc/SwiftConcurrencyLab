...
final class ConversationCell: UITableViewCell {
  ...
  override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
    ...
    avatar.font = .boldSystemFont(ofSize: 14)
  ...
  func configure(with item: ConversationSnapshot, delayMilliseconds: Int? = nil) {
    avatarTask?.cancel()
    ...
    content.secondaryText = item.preview
    content.image = nil
    content.imageToTextPadding = 48
    self.contentConfiguration = content
    let id = item.id
    let token = item.avatarToken
    avatarTask = Task { [weak self] in
      let delay = delayMilliseconds ?? (id == "engineering" ? 45 : 10)
      try? await Task.sleep(for: .milliseconds(delay))
      guard
        CellReuseIdentity.canCommit(
          cancelled: Task.isCancelled,
          representedID: self?.representedID,
          resultID: id
        )
      else { return }
      self?.avatar.text = token
      self?.avatar.backgroundColor = .systemIndigo
      self?.accessibilityValue = "avatar=\(token)"
    }
  }
  override func prepareForReuse() {
    ...
    representedID = nil
    avatar.text = "…"
    accessibilityValue = "avatar=pending"
  }
