...
final class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        let tabs = UITabBarController(); tabs.viewControllers = [inbox, learn]
        let window = UIWindow(windowScene: windowScene); window.rootViewController = tabs; self.window = window; window.makeKeyAndVisible()
    }
}
